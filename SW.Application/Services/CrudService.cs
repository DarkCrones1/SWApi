...
public class CrudService<T> : ICrudService<T> where T : BaseEntity
{
    ...
    protected virtual ICrudRepository<T> GetRepository()
    {
        ...
        if (typeRep == typeof(DataDream))
            return (ICrudRepository<T>)this._unitOfWork.DataDreamRepository;

        if (typeRep == typeof(UserAccount))
            return (ICrudRepository<T>)this._unitOfWork.UserAccountRepository;

        if (typeRep == typeof(UserData))
            return (ICrudRepository<T>)this._unitOfWork.UserDataRepository;

        return (ICrudRepository<T>)this._unitOfWork.UserDataCommendRepository;
    }
